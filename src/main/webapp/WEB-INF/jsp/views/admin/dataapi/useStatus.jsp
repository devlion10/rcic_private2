            ...
            <div class="content-panel">
                ...
                <div class="col-md-12 searchMB">
                    <form class="form-inline" role="form" id="searchForm" onsubmit="return false">
                        ...
                        <div class="form-group mr10">
                            <label class="mr10">API명</label>
                            <select class="form-control" name="apiProvdNo" id="apiProvdNo">
                                <option value="">전체</option>
                                <option value="1">고속도로 공사현황</option>
                                <option value="2">국도 공사현황</option>
                                <option value="3">지방도/기타 공사현황</option>
                                <option value="4">공사종류 및 시설종류 코드목록</option>
                            </select>
                        </div>
                ...
                <div class="total">
                    <button type="button" class="btn btn-theme04 btnSearch" onclick="exportTableToExcel('dataTable', 'rcic_api_stats')"><i class="fa fa-file-excel-o" aria-hidden="true" ></i></button>
                </div>
...
<script type="text/javascript">
    ...
    // FIXME
    var allApiList = [
        {apiProvdNo: 1,apiInfoNm: '고속도로 공사현황'},
        {apiProvdNo: 2,apiInfoNm: '국도 공사현황'},
        {apiProvdNo: 3,apiInfoNm: '지방도/기타 공사현황'},
        {apiProvdNo: 4,apiInfoNm: '공사종류 및 시설종류 코드목록'}
    ];
    ...
    function retrieve() {
        ...
        var targetHeaders = allApiList;
        if ($('#apiProvdNo').val() !== '') {
            targetHeaders = [{apiProvdNo: $('#apiProvdNo').val(), apiInfoNm: $('#apiProvdNo option:selected').text()}]
        }
        ...
        $.ajax({
            ...
            _.each(dateList, function (d) {
                var useDeLabel = moment(d, groupFormat).format(groupFormat == 'YYYYMM' ? 'YYYY년 MM월' : 'YYYY년'); // FIXME
                var row = {
                ...
                _.each(targetHeaders, function (h) {
                    var targetApiDate = _.find(res, {useDe: d, apiProvdNo: parseInt(h.apiProvdNo) }); // FIXME parseInt .. ?
                    var valueByApi = targetApiDate != null ? targetApiDate.summary : 0;
                    row.values.push(valueByApi);
                    row.total += valueByApi;

                    chartRow[h.apiProvdNo + ''] = valueByApi;
                })
                tableData.push(row);
    ...
    function createThead(headers) {
        ...
        for (var i in headers) {
            var h = headers[i];
            $head.append($('<th/>').text(h.apiInfoNm));
        }
    ...
    function drawChart(headers, chartData) {
        ...
        am4core.ready(function() {
            ...
            _.each(headers, function (h) {
                createSeries(h.apiProvdNo + '', h.apiInfoNm);
            })
