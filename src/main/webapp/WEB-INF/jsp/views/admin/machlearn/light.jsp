    ...
    <div class="row mt">
        ...
        <div class="col-md-12">
            <div class="content-panel">
                <h4>공고수집 모델</h4>
                <div style="margin: 0 0 10px 10px;">RCIC에 수집된 공고를 머신러닝 기법으로 학습하여 추가적으로 공고를 수집하거나 잘못 수집된 공고를 판별합니다.</div>
                <hr>
                                    ...
                                    <div class="col-md-4">
                                        <div class="input-group">
                                            <select class="form-control" id="tab2Select">
                                                <option value="omission" selected>추가 수집할 공고</option>
                                                <option value="wrong">잘못 수집된 공고</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    function retrieve(currPage) {
        ...
        $.ajax({
            type: 'get',
            url: '/admin/mach-learn/light/log',
            ...
            if (res.list && res.list.length > 0) {
                ...
                $.each(res.list, function (idx, item) {
                    ...
                    var percent = item.lrnNm == 'fail' ? '' : '정확도 '+ parseFloat(item.lrnResult).toFixed(2)+'%';
    ...
    function runModel() {
        ...
        $.ajax({
            type: 'post',
            url: '/admin/mach-learn/light/runModelPy',
        }).done(function (res) {
            console.log(res);
            alert('학습모델이 실행되었습니다.');
            retrieve(1);
    ...
    function retrieve2(currPage) {
        ...
        $.ajax({
            type: 'get',
            url: '/admin/mach-learn/light/result/' + searchApi,
            contentType: 'application/json',
            data: param
        }).done(function (res) {
            console.log('response', res);
            if (res.list && res.list.length > 0) {
                ...
                $.each(res.list, function (idx, item) {
                    ...
                    var $tr = $('<tr/>')
                        .append($('<td/>').text(item.bidntceno))
                        .append($('<td/>').text(item.bidntceord))
                        .append($('<td/>').html($title).addClass('align-left'))
