     ...
     // 검색, SNS, 관심공사 클릭
		...
		// 관심공사 최신순 클릭
			...
			*/
			...
			MapData.btnClickEvent('corInfoBox');
			//왜 한번더 지워야 지워지지...? 
			...
			if($("#Indigo4").hasClass("active") === true){
				if($("#Indigo0").hasClass("active") === false){
					$("#Indigo0").trigger("click");
					$("#Indigo4").addClass("active");
				}
			}
		...
		// 우측 컨트롤 관심공사 아이콘 클릭
				...
				$(".corBookMark").hide();
				// 맵 상세정보 최소화 클릭
			}).on("click", ".roadViewCloseBtn", function(){
				$(".roadViewPopup").hide();
		...
		/* 지도 상단 툴팁 마우스 오버서 툴팁 */
		$(".erection").hover(function(e){
        	$(this).find(".erectionTip").show();
        },function() {
        	$(this).find(".erectionTip").hide();
        });

        /* 지도 상단 점용불허구간 레이어 툴팁 마우스 오버서 툴팁 */
//        $("#noPrmsnLayerBtn").hover(function(e){
//            $(this).find(".noPrmsnLayerList").show();
//        },function() {
//            $(this).find(".noPrmsnLayerList").hide();
//        });
		...
		$('.listDot').mouseout(function() { $.mFavorBoxHover('out')});
