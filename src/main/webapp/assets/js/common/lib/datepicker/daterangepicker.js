    ...
    $.fn.dateRangePicker = function(opt) {
        ...
        function generateYearElement(date, month) {
            ...
            if (!opt.yearSelect ||
                ...
                return '<div class="month-element">' + nameYear(fullYear) + '</div>';
            }
            ...
            for (var week = 0; week < 6; week++) {
                ...
                for (var day = 0; day < 7; day++) {
                    ...
                    var todayDivAttr = {
                        ...
                        'class': 'day ' + today.type + ' ' + today.extraClass + ' ' + (today.valid ? 'valid' : 'invalid') + ' ' + (highlightToday ? 'real-today' : '') +' '+ (day == 6 ? 'blue-day':'') +' '+ (day == 0 ? 'red-day':'')
                    };
        ...
        function setMaxDate(maxDate){
            if (typeof maxDate === 'string')
                opt.endDate = moment(maxDate, this.locale.format);

            if (typeof maxDate === 'object')
                opt.endDate = moment(maxDate);

            if (opt.endDate && compare_day(opt.endDate, maxDate) == 0) {
                this.startDate = opt.endDate.clone();
            }
        }
        
        function setMinDate(minDate){
            if (typeof minDate === 'string')
                opt.startDate = moment(minDate, this.locale.format);

            if (typeof minDate === 'object')
                opt.startDate = moment(minDate);

            if (opt.startDate && compare_day(opt.startDate, minDate) < 0) {
                this.startDate = opt.startDate.clone();
            }
